...
import { FW_STRIKE_ANIM_SCALE } from './animScale'

/** Ambient field strike with a brief outline football flash on impact. */
export function runKomorebiStrikeVfx(opts: {
  ...
  return new Promise((resolve) => {
    ...
    const impactBall = track(document.createElement('div'))
    impactBall.className = `game-fw-impact-ball${faceHit ? ' game-fw-impact-ball--face' : ''}`
    impactBall.style.left = `${targetX}px`
    impactBall.style.top = `${targetY}px`
    fxLayer.appendChild(impactBall)

    const tl = gsap.timeline({
      timeScale: 1 / FW_STRIKE_ANIM_SCALE,
      onComplete: () => {
    ...
    tl.fromTo(
      impactBall,
      { opacity: 0, scale: 0.45, rotation: -18 },
      { opacity: 1, scale: 1, rotation: 0, duration: 0.12, ease: 'back.out(2.2)' },
      0.32,
    )
    tl.to(
      impactBall,
      { opacity: 0, scale: 1.35, rotation: 24, duration: 0.38, ease: 'power2.in' },
      0.44,
    )
