...
export const Game: React.FC<Props> = ({
  ...
  const { activeBanner } = useTurnBanners(
    match,
    endTurnVisual,
    combatResult,
    processing,
  ...
  useEffect(() => {
    const handleKeydown = (event: KeyboardEvent) => {
      ...
      if (event.key.toLowerCase() === 'e') { event.preventDefault(); handleEndTurn(); }
      if (event.key.toLowerCase() === 'b') { event.preventDefault(); handleBattle(); }
  ...
  return (
    <GameScalableContainer>
      <div className="game-stage" data-gameplay-variant={getGameplayVariant()} ref={stageRef}>
          ...
          <div className="game-board-lane game-board-lane--opponent">
            {boardMatch.villain.board.map((unit, slot) => (
              <div
                ...
                ref={(el) => { villainBoardRefs.current[slot] = el; }}
              >
                {unit?.display ? (
                  <div
                  ...
                  >
                    <Card
                      {...unit.display}
                      stats={{ ...unit.display.stats, attack: unit.attack, health: displayHealth(unit.health) }}
                      totalCards={1}
                      fanIndex={0}
                      layoutMode="compact"
          ...
          <div className="game-board-lane game-board-lane--hero" ref={heroBoardLaneRef}>
            {boardMatch.hero.board.map((unit, slot) => (
              <div
                key={`h-board-${slot}`}
                className={`game-board-slot${unit ? '' : ' game-board-slot--vacant'}`}
                ref={(el) => { heroBoardRefs.current[slot] = el; }}
              >
                {unit?.display ? (
                  <div
                    className="game-board-card-wrap"
        ...
        {activeBanner && activeBanner.phase !== 'hidden' ? (
          <TurnBanner
            variant={activeBanner.variant}
            phase={activeBanner.phase === 'exit' ? 'exit' : 'enter'}
            config={
              activeBanner.variant === 'enemy'
                ? gameAnimationsConfig.turnBanner.enemy
                : gameAnimationsConfig.turnBanner.your
            }
          />
        ) : null}
